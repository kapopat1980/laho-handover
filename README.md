...
It is evaluated against three baselines (fixed-threshold, dwell-time-and-score, speed-only fuzzy-adaptive) across 6 mobility/congestion scenarios, each run over **20 random seeds**.

## Blocking model validation

The congestion-dependent blocking rule's realism was cross-checked directionally against the classical Erlang-B loss formula (Section 5.5 of the paper): both the simulator's blocked-handover percentage and the Erlang-B formula increase monotonically with congestion, though the simulator uses a deliberate threshold rule rather than one numerically calibrated to Erlang-B. Recalibrating the blocking model against Erlang-B directly is noted as future work.
...
├── requirements.txt
├── paper/
│   └── LAHO_Location_Aware_Handover_Paper.docx        # earlier IEEE Access-formatted version (not accepted)
├── paper_springer/
│   ├── LAHO_Discover_IoT_Springer_Format.docx          # current version, submitted to Discover IoT
│   └── build_springer.js                               # docx-generation script for the Springer version
├── src/
│   ├── sim.py            # main simulator: 4 algorithms × 6 scenarios × 20 seeds
...
└── figures/               # all 10 figures as generated (PNG, 200 dpi)
```
...
# 1. Run the main simulation (4 algorithms × 6 scenarios × 20 seeds)
python sim.py                 # writes ../results/results.json

# 2. Aggregate results and generate Tables 3/4 and Figs. 4-7
...
# 4. Run handover event classification + sensitivity analysis (Table 5, Fig. 8-9)
python sim_extra.py           # writes ../results/event_totals.json, ../results/sensitivity.json
python figures3.py

# 5. Run statistical significance, computational overhead, and scalability analysis (Table 6, Fig. 10)
python rigor_extra.py         # writes ../results/statistics.json, runtime.json, scalability.json
python figures4.py
```

All randomness is seeded (`SEEDS = list(range(20))` in `sim.py`/`sim_extra.py`/`rigor_extra.py`), so re-running the scripts above reproduces the exact numbers reported in the paper. Note: `sim.py` (480 runs) takes a few minutes, and `sim_extra.py` (~780 runs, event classification + parameter sensitivity) and `rigor_extra.py` each take approximately 5-6 minutes on typical hardware.

## Figure/table → script mapping

| Paper item (Discover IoT numbering) | Generated by |
|---|---|
...
| Table 3, 4; Fig. 4 (overall), Fig. 5 (stress scenario), Fig. 6 (congestion trend) | `src/sim.py` + `src/analyze.py` |
| Fig. 7 (throughput across scenarios) | `src/analyze.py` |
| Table 5; Fig. 8 (event classification) | `src/sim_extra.py` + `src/figures3.py` |
| Fig. 9 (β / θ_L sensitivity) | `src/sim_extra.py` + `src/figures3.py` |
| Table 6 (computational overhead); Fig. 10 (scalability sweep) | `src/rigor_extra.py` + `src/figures4.py` |
...
| UEs per scenario | 30 (see Fig. 10 for a 10-100 UE scalability sweep) |
